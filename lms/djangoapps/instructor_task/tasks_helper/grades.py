...
class GradeReportBase(object):
    ...
    def _batch_users(self, context):
        ...
        def get_enrolled_learners_for_course(course_id, verified_only=False):
            ...
            """
            self.log_additional_info_for_mit_course(
                context,
            ...
            for user_ids in user_chunks:
                ...
                self.log_additional_info_for_mit_course(context, 'ProblemGradeReport: user chunk yielded successfully')
                yield users
    ...
    def log_additional_info_for_mit_course(self, context, message):
        """
        Investigation logs for MIT course.

        TODO -- Remove as a part of PROD-1287
        """
        if context.log_additional_info:
...
class _ProblemGradeReportContext(object):
    ...
    def __init__(self, _xmodule_instance_args, _entry_id, course_id, _task_input, action_name):
        ...
        self.log_additional_info = six.text_type(self.course_id) == 'course-v1:MITx+CTL.SC0x+1T2019'
...
class ProblemGradeReport(GradeReportBase):
    ...
    def _rows_for_users(self, context, users):
        ...
        Returns a list of rows for the given users for this report.
        """
        self.log_additional_info_for_mit_course(context, 'ProblemGradeReport: Start of row creation for users')
        success_rows, error_rows = [], []
        for student, course_grade, error in CourseGradeFactory().iter(
            ...
            if context.task_progress.attempted % 2 == 0:
                self.log_additional_info_for_mit_course(
                    context,
                    'ProblemGradeReport: Total Processed:{}'.format(context.task_progress.attempted)
                )
            if not course_grade:
